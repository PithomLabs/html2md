...
- `main.go:17-68` — CLI entrypoint. Reads HTML, runs pipeline, writes `.md`.
- `main.go:70-75` — `computeOutputPath()`: same dir, `.html` → `.md`.
...
- `converter/markdown.go:13-18` — `ToMarkdown()`: entry point for conversion.
- `converter/markdown.go:21-88` — `processNode()`: recursive DOM walker with tag switch.
- `converter/markdown.go:187-233` — `writeList()`: handles ordered/unordered lists with `start` attr.
- `converter/markdown.go:354-428` — Inline text extraction via `golang.org/x/net/html` nodes.
...
- Relative URLs resolved against hardcoded `https://www.expressvpn.com` (see `converter/markdown.go:169,407`).
...
3. For inline elements, handle in `extractInlineNode()` (`converter/markdown.go:380`).
...
3. Add `writeYAMLField()` call in `writeFrontmatter()` (`main.go:77`).

## Known Issues

- **Hardcoded base URL** (`converter/markdown.go:169,407`): relative `/path` URLs resolve to `https://www.expressvpn.com`. Should be a CLI flag or config.
